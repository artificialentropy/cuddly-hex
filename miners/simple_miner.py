...
# node helpers (must be importable)
...
def normalize_tx_for_network(tx: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        if sig and isinstance(sig, (list, tuple)) and sig and not isinstance(sig[0], str):
            try:
...
def main():
    ...
    try:
        while True:
            ...
            header_version = int(tip.get("version", 1) or 1)
            ...
            if not found:
                log_every = 50000
                while mc.hex_to_binary(cur_hash)[: difficulty] != target:
                    nonce += 1
