...
#     # core/block.py  (inside Block class)

#     def to_json(self):
#         """
#         Serialize this Block instance as a JSON-serializable dict.
#         NOTE: intentionally does NOT include a `version` key.
#         """
#         return {
...
#             # keep height optional for convenience (useful for debugging / storage)
...
#         This is defensive: we only consume known keys so extra fields (like 'version')
#         do not cause constructor errors.
#         """
#         if not isinstance(block_json, dict):
#             raise TypeError("block_json must be a dict")

#         # allowed keys that map to Block.__init__ parameters
#         allowed = {"timestamp", "last_hash", "hash", "data", "difficulty", "nonce", "merkle", "height"}

#         sanitized = {k: v for k, v in block_json.items() if k in allowed}

#         # provide sensible defaults if missing
#         timestamp = int(sanitized.get("timestamp", 0))
#         last_hash = sanitized.get("last_hash", "")
#         h = sanitized.get("hash", "")
#         data = sanitized.get("data", []) or []
#         difficulty = int(sanitized.get("difficulty", MIN_DIFFICULTY))
#         nonce = int(sanitized.get("nonce", 0))
#         merkle = sanitized.get("merkle", None)
#         height = sanitized.get("height", None)

#         return Block(timestamp, last_hash, h, data, difficulty, nonce, merkle, height)
...
def _median_time_past(recent_blocks: List["Block"]) -> int:
    ts = [b.timestamp for b in recent_blocks[-11:]]
    return int(median(ts)) if ts else 0
